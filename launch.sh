...
if ! [[ -f 'ATM7-1.0.3-server.zip' ]]; then
	rm -fr defaultconfigs config kubejs libraries mods Simple.zip forge*.jar
	curl -Lo 'ATM7-1.0.3-server.zip' 'https://edge.forgecdn.net/files/5100/970/ATM7-1.0.3-server.zip' && unzip -u -o 'ATM7-1.0.3-server.zip' -d /data
	if [[ -d $(echo ATM7-1.0.3-server.zip | sed 's/.zip//') ]]; then
		mv -f $(echo ATM7-1.0.3-server.zip | sed 's/.zip//')/* /data
		rm -fr $(echo ATM7-1.0.3-server.zip | sed 's/.zip//')
	fi
